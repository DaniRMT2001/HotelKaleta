  ...
  $.ajax({
    ...
    success: function (datos) {
      ...
      if (datos == 'ok') {
        $("#contenido").html(' ');
        raeadImg();
        window.location = "canseladas.html";
      }
  ...
  $.ajax({
    ...
    success: function (datos) {
      ...
      $(datos).each(function (index, tabla) {
        ...
        $("#contenido").append("<div class='card mt-2 bg-dark border border-white' style='width: 18rem;'><img id='img'src='../" + datos[index].imagen + "' class='card-img-top' height='170px' width='280px'><div class='card-body '> <h5 class='card-title'>" + datos[index].tipo_hab + "</h5><p class='card-text'> Disponible: " + cantidad + "</p></div></div>")
